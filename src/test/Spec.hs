...
import Echidna.Types.Campaign (Campaign, CampaignConf(..), TestState(..), tests, gasInfo, testLimit, shrinkLimit, knownCoverage)
...
testContract' fp n cfg s as = testCase fp $ do
  ...
  let c' = c & sConf . quiet .~ True
             ...
             & (if s then cConf . shrinkLimit .~ 2500 else id)
